...
# Create the mappings for each category. dict(zip()) creates a dictionary from two lists. The first list is the keys, the second list is the values. The keys are the drawing numbers, the values are the descriptions. This works!
body_mapping = dict(zip(df['Body drawings'].dropna().str.strip(), df['Body description'].dropna().str.strip()))
piston_mapping = dict(zip(df['Piston drawings'].dropna().str.strip(), df['Piston description'].dropna().str.strip()))
nrv_mapping = dict(zip(df['NRV drawings'].dropna().str.strip(), df['NRV description'].dropna().str.strip()))
